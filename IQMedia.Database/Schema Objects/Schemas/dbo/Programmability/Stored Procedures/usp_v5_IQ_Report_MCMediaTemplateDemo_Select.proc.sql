CREATE PROCEDURE [dbo].[usp_v5_IQ_Report_MCMediaTemplateDemo_Select]
...
BEGIN
	...
	DECLARE @ClientResult VARCHAR(MAX)
	...
	SELECT	@FromDate = CASE ISNULL(tbl.s.value('FromDate[1]', 'varchar(100)'), '') WHEN '' THEN NULL ELSE tbl.s.value('FromDate[1]', 'datetime') END,
			...
			@ClientResult = CASE ISNULL(tbl.s.value('ClientGuid[1]', 'varchar(max)'), '') WHEN '' THEN NULL ELSE tbl.s.value('ClientGuid[1]', 'varchar(max)') END
	...
	IF (@SearchSettings.exist('SearchSettings/CategorySet[@IsAllowAll="true"]') = 1)
	  BEGIN
		...
		WHERE (@ClientResult IS NULL OR IQArchive_Media.ClientGUID = @ClientResult)
		...
		BEGIN
			...
			BEGIN
				...
				WHERE IQArchive_Media.CategoryGUID in (SELECT t1.CategoryGUID FROM @TempTable t1)
					AND (@ClientResult IS NULL OR IQArchive_Media.ClientGUID = @ClientResult)
					AND IQArchive_Media.IsActive = 1
					AND IQArchive_MCMedia.IsActive = 1
					AND	((@FromDate IS NULL OR @ToDate IS NULL) OR IQArchive_Media.MediaDate BETWEEN @FromDate AND @ToDate) 
					AND	(@SearchTerm IS NULL OR (Content LIKE '%' + @SearchTerm + '%' OR Title LIKE '%' + @SearchTerm + '%'))
	...
	IF @ReportXML IS NOT NULL
		BEGIN		
			...
			WHERE	tblIQMedia_Archive.CategoryName IS NOT NULL
			GROUP	BY tblIQMedia_Archive.CategoryName
			ORDER	BY tblIQMedia_Archive.CategoryName
