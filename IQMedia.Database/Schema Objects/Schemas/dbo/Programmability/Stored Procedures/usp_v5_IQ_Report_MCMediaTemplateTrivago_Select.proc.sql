...
/****** Object:  StoredProcedure [dbo].[usp_v4_IQ_Report_MCMediaTemplateTrivago_Select]    Script Date: 12/14/2015 8:48:38 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_v5_IQ_Report_MCMediaTemplateTrivago_Select]
...
BEGIN
	...
	DECLARE @SentimentFlag INT

	SELECT	@FromDate = CASE ISNULL(tbl.s.value('FromDate[1]', 'varchar(100)'), '') WHEN '' THEN NULL ELSE tbl.s.value('FromDate[1]', 'datetime') END,
			@ToDate = CASE ISNULL(tbl.s.value('ToDate[1]', 'varchar(100)'), '') WHEN '' THEN NULL ELSE tbl.s.value('ToDate[1]', 'datetime') END,
			@SearchTerm = CASE ISNULL(tbl.s.value('SearchTerm[1]', 'varchar(max)'), '') WHEN '' THEN NULL ELSE tbl.s.value('SearchTerm[1]', 'varchar(max)') END,
			...
			@sentimentFlag = CASE ISNULL(tbl.s.value('SentimentFlag[1]', 'varchar(2)'), '') WHEN '' THEN NULL ELSE tbl.s.value('SentimentFlag[1]', 'int') END
	...
	IF (@SearchSettings.exist('SearchSettings/CategorySet') = 0 OR @SearchSettings.exist('SearchSettings/CategorySet[@IsAllowAll="true"]') = 1)
	  BEGIN
	...
	IF @ReportXML IS NOT NULL
		BEGIN		
			...
			SELECT 
				...
				tblIQMedia_Archive.MediaType,
				tblIQMedia_Archive.SubMediaType,
				ArchiveRadio.ClipGuid,
				ArchiveRadio.LocalDatetime,
				IQ_Station.Dma_Name,
			...
			INNER JOIN ArchiveRadio WITH (NOLOCK)
				ON tblIQMedia_Archive._ArchiveMediaID = ArchiveRadio.ArchiveRadioKey
				AND	tblIQMedia_Archive.SubMediaType = ArchiveRadio.v5SubMediaType
			INNER	JOIN IQ_Station
				ON IQ_Station_ID = SUBSTRING(ArchiveRadio.IQ_CC_Key,0 ,CHARINDEX('_',ArchiveRadio.IQ_CC_Key))
			INNER JOIN IQ_MediaTypes
				ON IQ_MediaTypes.SubMediaType = tblIQMedia_Archive.SubMediaType
				AND IQ_MediaTypes.TypeLevel = 2
			...
			SELECT	'PosSentimentFilter' AS TableType,
					COUNT(1) AS NumResults
			FROM	@tblIQMedia_Archive
			WHERE	PositiveSentiment > 0

			SELECT	'NegSentimentFilter' AS TableType,
					COUNT(1) AS NumResults
			FROM	@tblIQMedia_Archive
			WHERE	NegativeSentiment > 0

			SELECT	'NullSentimentFilter' AS TableType,
					COUNT(1) AS NumResults
			FROM	@tblIQMedia_Archive
			WHERE	ISNULL(PositiveSentiment, 0) = 0
					AND ISNULL(NegativeSentiment, 0) = 0
		END
END