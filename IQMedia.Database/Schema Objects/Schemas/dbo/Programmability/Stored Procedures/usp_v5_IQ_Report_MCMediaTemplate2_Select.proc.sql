CREATE PROCEDURE [dbo].[usp_v5_IQ_Report_MCMediaTemplate2_Select]
...
BEGIN
	...
	SELECT	@FromDate = CASE ISNULL(tbl.s.value('FromDate[1]', 'varchar(100)'), '') WHEN '' THEN NULL ELSE tbl.s.value('FromDate[1]', 'datetime') END,
			...
			@ClientGUID = CASE ISNULL(tbl.s.value('ClientGuid[1]', 'varchar(100)'), '') WHEN '' THEN NULL ELSE tbl.s.value('ClientGuid[1]', 'uniqueidentifier') END,
	...
	IF (@SearchSettings.exist('SearchSettings/CategorySet[@IsAllowAll="true"]') = 1)
	  BEGIN
		...
		WHERE 
			...
			AND (@ClientGUID IS NULL OR ClientGUID = @ClientGUID)
	  END
	ELSE
	  BEGIN
